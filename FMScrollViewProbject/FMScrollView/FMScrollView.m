...
//        [self showmask:(FMScrollViewCell *)gestureRecognizer.view];
    ...
    [[ScrollViewPressControl shareInfo] removeScrollViewPressAction:TAP_PRESS_VIEW];
}
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer{
    ...
    if ([[ScrollViewPressControl shareInfo] isExistScrollViewPressAction:TAP_PRESS_VIEW]){
        return NO;
        
    }else{
        Class gestureClass = [gestureRecognizer class];
        if ([gestureClass isSubclassOfClass:[FMScrollViewTapGestureRecognizer class]]) {
             [[ScrollViewPressControl shareInfo] addScrollViewPressAction:TAP_PRESS_VIEW];
        }
        return YES;
    }
...
@implementation ScrollViewPressControl

static ScrollViewPressControl *_ScrollviewPressControl = nil;

+ (ScrollViewPressControl *)shareInfo{
    @synchronized(self){
        if (!_ScrollviewPressControl) {
            _ScrollviewPressControl = [[self alloc] init];
        }
    }
    return _ScrollviewPressControl;
}

+ (void)freeInfo{
    if (_ScrollviewPressControl) {
        _ScrollviewPressControl=nil;
    }
}
...
- (id)init{
    self = [super init];
    if (self) {
        _arrayScrollviewPressView = [[NSMutableArray alloc] init];
    }
    return self;
}

- (void)addScrollViewPressAction:(PRESS_VIEW)view{
   [_arrayScrollviewPressView addObject:[NSString stringWithFormat:@"%d", view]];
}
- (void)removeScrollViewPressAction:(PRESS_VIEW)view{
    if ([_arrayScrollviewPressView containsObject:[NSString stringWithFormat:@"%d", view]]) {
        [_arrayScrollviewPressView removeObject:[NSString stringWithFormat:@"%d", view]];
    }
}
- (BOOL)isExistScrollViewPressAction:(PRESS_VIEW)view{
    return [_arrayScrollviewPressView containsObject:[NSString stringWithFormat:@"%d", view]];
}
