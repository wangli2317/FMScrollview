...
- (CGFloat)cellHeightForIndexPath:(NSIndexPath *)indexPath model:(id)model keyPath:(NSString *)keyPath
{
    ...
    if (cacheHeight) {
        ...
        if (self.modelTableview && self.modelTableview != self.modelCell.fm_tableView
            ) {
            self.modelCell.fm_tableView = self.modelTableview;
        }
        self.modelCell.fm_indexPath = indexPath;
        ...
        if (self.modelCell.fm_indexPath && self.modelCell.fm_tableView) {
            [self.modelCell.contentView.autoLayoutModelsArray enumerateObjectsUsingBlock:^(FMAutoLayoutModel *model, NSUInteger idx, BOOL *stop) {
                [self.modelTableview.cellAutoHeightManager setSubviewFrameCache:model.needsAutoResizeView.frame WithIndexPath:self.modelCell.fm_indexPath];
            }];
        }
